...
describe('TransactionService', () => {
  ...
  describe('getTransactionsForCompany', () => {
    ...
    describe('with custom query options', () => {
  ...
  describe('getTransactionPreviewForCompany', () => {
    it('returns preview items and remaining count', async () => {
      ...
      // verify that last item is not included in the response array
    ...
    describe('when no transactions found', () => {
      it('queries preview transactions for company and user with limit', async () => {
        ...
        const response = await TransactionService.getTransactionPreviewForCompany(
          'cmp_123',
          'user_123',
          2
        );
    ...
    describe('when DB query fails', () => {
      it('logs error and throws if DB fails', async () => {
        ...
        expect(logErrorSpy).toHaveBeenCalledWith(
          'TransactionService',
          expect.stringContaining('Error querying transaction preview'),
          expect.any(Error)
        );
      });
    });
